...
module SenselessC {
  uses {
	...
	interface Timer<TMilli> as Anchortimer;			// Anchor Timer
	interface Leds;									// Leds
	...
	// Calibration
	//---------------------------------------------------------------------------
	interface AMSend as CalibrationSend;			// Used to transmit beacon of an anchor to another anchor
	interface Receive as CalibrationReceive;		// Used to receives these beacons
  }
}
implementation {
  ...
  task void BroadcastTask();			// Task for transmitting broadcast message to blind nodes
  task void CalibrationTask();			// // Task for transmitting broadcast message to anchor nodes
  ...
  // Add a node to the linked list, this is used by a blind node to know to which nodes it is connected to:
  // * Parameter 1: pointer to the linked list
  // * Parameter 2: the id of the node that you want to add
  // * Parameter 3: Virtual anchor node or not 
  //---------------------------------------------------------------------------
  struct node *addToList(struct node **p, uint8_t id, uint8_t VANs)
{
  ...
  // Check if the node is in range of 3 anchor or virtual anchor nodes
  // * Parameter 1: pointer to the linked list
  //---------------------------------------------------------------------------
struct node **VAN(struct node **n)
{
  ...
  // Search in the list for a node with the id
  // * Parameter 1: pointer to the linked list
  // * Parameter 2: the id of the node that you want to search
  // * Parameter 3: Virtual anchor node or not 
  //---------------------------------------------------------------------------
  ...
  // Check which nodes are inactive in the network
  // * Parameter 1: pointer to the linked list
  //---------------------------------------------------------------------------
	struct node **InactiveNodes(struct node **n)
	{
		while (*n != NULL)
		{
			if ((*n)->Checked == FALSE)
  ...
  // To check if nodes are inactive, we set their status to false when the timer fires
  // If no packet is received in a certain time then the nodes will be removed from the list
  // * Parameter 1: pointer to the linked list
  //---------------------------------------------------------------------------
	void SetStatusToFalse(struct node **n)
	{
		while (*n != NULL)
		{
				(*n)->Checked = TRUE;
	...
	else 
	{
      ...
      if (call Timer.isRunning())
		...
		if(!Root)
		{
			...
			call Anchortimer.startPeriodic(60000);
		}
  ...
  LocMsg_t *out;
	...
	printf("VANr van deze node is: %u\n",LocalLocMsg.VANr);
	  ...
	  if (!Root && LocalStatMsg.active && !LocalStatMsg.AN ) 
	  {
  ...
  // Event handled when an anchor node receives a broadcast message from other anchor nodes:
  // * Checks if the radio is free to transmit the message with the collection protocol
  // * Else the message is put on a queue
  //---------------------------------------------------------------------------
  event message_t* CalibrationReceive.receive(message_t* msg, void* payload, uint8_t len) {
  
  broadcast_t *omsg = (broadcast_t*)payload;


	printf(" Received calibration message from node: %u\n", omsg->id);
  ...
  // Task for sending a broadcast message to the anchor nodes by an anchor node:
  // * When the message is being sent, then the radio is busy and the task is finished
  //---------------------------------------------------------------------------
  task void CalibrationTask()
  {
		if (call CalibrationSend.send(AM_BROADCAST_ADDR, &SndMsgB, sizeof(broadcast_t)) == SUCCESS)
		{
		  SendBusy = TRUE;
		  TaskBusy = FALSE;
		  printf("CalibrationSend.send == SUCCESS\n");
		  call PrintfFlush.flush();
		}
		else 
			ReportProblem();
  ...
  // Event handled when the Anchortimer fires:
  // This timer has two function:
  // * Sending broadcast messages to other anchor nodes
  // * Deleting old nodes in the linked list
  //---------------------------------------------------------------------------
    event void Anchortimer.fired(){
		if ( !SendBusy && !Root && LocalStatMsg.active && LocalStatMsg.AN )
		{
			broadcast_t * o = (broadcast_t *)call CalibrationSend.getPayload(&SndMsgB);
			memcpy(o, &local, sizeof(broadcast_t));
			TaskBusy = TRUE;
			post CalibrationTask();
		}
		else
		  ReportProblem();
	  
	    if(CheckForOldAnchors)
		{
			list_remove(InactiveNodes(&n));
		}
		else
			SetStatusToFalse(&n);
			
		CheckForOldAnchors = !CheckForOldAnchors;
  ...
  // Event handled when the broadcastsend is successfully done:
  //---------------------------------------------------------------------------
  event void CalibrationSend.sendDone(message_t* msg, error_t error) {
    if (error == SUCCESS)
	{
      ReportSent();
		printf("CalibrationSend.sendDone\n");
		call PrintfFlush.flush();
	}
    else
      ReportProblem();
