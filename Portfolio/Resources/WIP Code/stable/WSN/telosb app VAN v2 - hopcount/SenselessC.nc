...
struct node **VAN(struct node **n)
{
	...
	if (ans	== 3)
	return 1;
	else if( ans == 2 && vans >= 1)
	return 2;
	else if (ans == 1 && vans >= 2)
	return 3;
	else if (ans == 0 && vans >= 3)
	return 4;
	else 
	return NULL;
  ...
  */
  ...
  LocalStatMsg.power = call CC2420Packet.getPower(msg);
	...
	printf("VANr van deze node is: %u\n",LocalLocMsg.VANr);
		...
		if(!LocalStatMsg.AN && (list_search( &n, LocalLocMsg.ANmoteid, LocalLocMsg.VANs) == NULL) )
	  ...
	  if (!Root && LocalStatMsg.active && (LocalLocMsg.VANs > 0) ) 
	  {
		...
		else
		{
			...
			if(!LocalStatMsg.AN && LocalLocMsg.VANs == 1)
			{
				local.id = LocalLocMsg.ANmoteid;
				local.hopCount = LocalLocMsg.hopCount + 1;
				local.VAN = LocalLocMsg.VANs;
				post BroadcastTask();
			}
		}
	  }
  ...
  event void BroadcastTimer.fired() {
	...
	if ( (!SendBusy && !Root && LocalStatMsg.AN) || (LocalLocMsg.VANr>0) )
	  {
