    ...
    public void messageReceived(int dest_addr, Message message)
    {
        ...
        if (message instanceof CollMsg)
        {
            ...
            for (int i = 0; i< length; i++)
            {
               ...
               */
            if (Nodes.contains(CollectedMsg.get_moteid()) == false)
                Nodes.add(CollectedMsg.get_moteid());
            ...
            //if (convertHumidity(CollectedMsg.get_humidityreading()) < 0)
              //  System.out.println("message from node without sensors");
        ...
        if (message instanceof LocMsg)
        {
            ...
            if (Nodes.contains(LocationMsg.get_moteid()) == false)
                Nodes.add(LocationMsg.get_moteid());
        ...
        else if (message instanceof StatMsg)
        {
            ...
            if (Nodes.contains(StatusMsg.get_moteid()) == false)
                Nodes.add(StatusMsg.get_moteid());
            ...
            //if(StatusMsg.get_type() == 3)  //send if node reboots
            //{
            ...
            else if(StatusMsg.get_type() == 4) //Reply on request
            {
                // fill up xml4
                xml4 xmlParser3 = new xml4();
                xmlParser3.set_moteID(StatusMsg.get_moteid());
                ...
                try
                {
                    ...
                    wr.write(xmlString3);
                    wr.flush();
                    System.out.println("status/reply message sent to controller");
                } catch (IOException e) { e.printStackTrace(); }
