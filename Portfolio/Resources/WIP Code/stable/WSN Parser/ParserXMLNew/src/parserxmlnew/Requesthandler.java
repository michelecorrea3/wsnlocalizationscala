...
public class Requesthandler implements Runnable {
    ...
    public void createListener() throws IOException {
        System.out.println("Creating socket on port 3801");
        this.listenSocket = new ServerSocket(this.port);
        System.out.println("Created socket on port 3801");
    }
	...
	try {
        ...
        while (true) {
            msg = new String();
            try {
                System.out.println("Listening on 3801");
                msg = readConnection();
                System.out.println("Message received over 3801");
            } catch (IOException ex) { ex.printStackTrace(); }
            handle(msg);
        }
    }
