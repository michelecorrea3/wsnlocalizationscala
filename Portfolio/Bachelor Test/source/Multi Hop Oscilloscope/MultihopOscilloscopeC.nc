...
implementation {
  ...
  //array om de sensordata in te kunnen opslaan vooraleer ze in local.readings komen
  nx_uint16_t readingsa[NREADINGS];
  nx_uint16_t readingsb[NREADINGS];
  //nodig om de count correct te kunnen incrementeren	
  uint16_t counta = 0;
  uint16_t countb = 0;
  ...
  */
  event void Timer.fired() {
	...
	if (call Send.send(&sendbuf, sizeof(local)) == SUCCESS){
	  sendbusy = TRUE;
	  printf("Succesvol pakket van sensor A verzonden!\n");
            
	}
        else{
          report_problem();
	  printf("Pakket van sensor A is niet verzonden!\n");
	}			
  ...
  if (readingb == NREADINGS) {
      printf("Aantal metingen bereikt\n");
      i = 0;
      local.count = countb;
      local.id = TOS_NODE_ID + 1;
      printf("id = %u\n",local.id);	
      //call PrintfFlush.flush(); 		  
     while(i<NREADINGS){	
	local.readings[i] = readingsb[i];
	i++;	
      }
      printf("sendbusy = %u\n",sendbusy);      
      if (!sendbusy) {
	...
	if (call Send.send(&sendbuf, sizeof(local)) == SUCCESS){
	  sendbusy = TRUE;
	  printf("Succesvol een pakket van sensor b verzonden!\n");
            
	}
        else{
          report_problem();
	  printf("Pakket van sensor b is niet verzonden!\n");
	}			
      if (!suppress_count_change)  
	countb++;
      suppress_count_change = FALSE;  		
