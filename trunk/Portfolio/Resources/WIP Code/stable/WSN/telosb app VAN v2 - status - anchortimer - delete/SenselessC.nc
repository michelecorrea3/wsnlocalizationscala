	...
	else 
	{
      ...
      if (call Timer.isRunning())
		...
		if(!Root)
		{
			...
			call Anchortimer.startPeriodic(600000);
		}
	  ...
	  if (!Root && LocalStatMsg.active && (omsg->VAN > 0) ) 
	  {
		...
		else
		{
			...
			if (!SendBusy) 
			{
			  ...
			  TaskBusy = TRUE;
			  if( (LocalLocMsg.VANr == 1 && AnchorMaySend ==TRUE) || LocalLocMsg.VANr == 0 || LocalLocMsg.VANr == 2)
			  {
				post RadioSendTaskL();
				if(LocalLocMsg.VANr == 0)
				AnchorMaySend = FALSE;
			  }
			  //TaskBusy = TRUE;
			}
